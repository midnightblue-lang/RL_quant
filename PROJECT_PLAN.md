...
在阶段 1 小样本数据链路和阶段 2 股票池/标签逻辑稳定后，把 AKShare 当前可获取的 A 股日频数据完整缓存到本地，并生成全市场下载审计和质量报告。该阶段只扩大数据覆盖，不改变清洗口径，不进入真实交易、RL 或 XGBoost。
...
- 全市场质量报告。
- 全量接入摘要报告。
- 基于全量缓存重建的 daily panel、月度股票池和未来收益标签。

### 执行步骤

1. 使用 AKShare 当前 A 股代码表生成股票列表，默认继续排除北交所。
2. 逐只下载日频行情，保存 raw 和 standard 两层 Parquet 缓存。
3. 支持断点续传，已经同时存在 raw 和 standard 缓存的股票默认跳过。
4. 定期写入 manifest，记录每只股票的状态、行数、缓存路径和错误信息。
5. 单独输出失败股票列表，便于后续重试或人工检查。
6. 对标准化缓存生成全市场质量报告。
7. 基于全量标准化缓存重新运行阶段 2，生成 daily panel、月度股票池和标签。

### 清洗和审计规则

- 沿用 `docs/DATA_CLEANING.md` 的口径，只做字段标准化、日期解析、数值类型转换、成交量单位统一和排序。
- 不前向填充 OHLCV，不伪造停牌交易日，不自动删除异常涨跌幅样本。
- 前复权 OHLC 与原始成交额/成交量计算出的 VWAP 可能口径不一致，只在质量报告中提示。
- 当前全量列表不等价于无幸存者偏差的历史全市场，退市股票和历史 ST 状态需要在后续研究中继续处理。

### 验收标准

- `download-full` 可以生成股票列表、manifest、失败列表和全市场质量报告。
- `stage25-full-data` 可以在全量缓存基础上重建阶段 2 输出。
- 中断后重新执行不会重复下载已经完整缓存的股票。
- 质量报告明确暴露异常数据，而不是静默修复或删除。
